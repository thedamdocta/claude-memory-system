#!/bin/bash
# PreToolUse hook — enforces Layer 1 router size cap
# Fires on: Write|Edit operations on router files in MyProject vault
# Exit 2 = block write that exceeds cap (unless shrinking)
# Exit 0 = pass
...
# Layer detection — only fire on router files
BASENAME=$(basename "$FILE_PATH")
case "$BASENAME" in
  _INDEX.md|_SESSION_LOG.md|MEMORY.md|_AI_SESSION_GUIDE.md) ;;
  _*_INDEX.md) ;; # Catch any _*_INDEX.md patterns
  *) exit 0 ;; # Not a router file
esac
